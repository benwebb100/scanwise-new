...
export const AIAnalysisSection: React.FC<AIAnalysisSectionProps> = ({
  ...
  // };
  // Around line 163, update the handleAddFinding function:
  const handleAddFinding = (detection: Detection, index: number) => {
    // Get tooth mapping if available
    const toothMapping = toothMappings[index];
    ...
    // Add suggested treatment based on condition
    const conditionName = normalizedDetection.class_name || normalizedDetection.class || '';
    const suggestedTreatment = getSuggestedTreatmentForCondition(conditionName);
    
    // Create an enhanced detection with suggested treatment
    const enhancedDetection = {
      ...normalizedDetection,
      suggestedTreatment
    };
    
    // Add to findings with tooth mapping and suggested treatment
    onAcceptFinding?.(enhancedDetection, toothMapping);
                          ...
                          try {
                            ...
                            activeConditions.forEach((detection) => {
                              ...
                              if (!addedDetections.has(originalIndex)) {
                                ...
                                // Normalize and add suggested treatment
                                const normalizedDetection = normalizeMissingToGeneric(detection);
                                const conditionName = normalizedDetection.class_name || normalizedDetection.class || '';
                                const suggestedTreatment = getSuggestedTreatmentForCondition(conditionName);
                                ...
                                onAcceptFinding?.(enhancedDetection as any, mapData as any);
                            ...
                            if (addedCount > 0) {
                              toast({ 
                                ...
                                description: `${addedCount} active condition${addedCount === 1 ? '' : 's'} added with suggested treatments. Please review and adjust treatments as needed.` 
                              });
                            }
                          } finally {
